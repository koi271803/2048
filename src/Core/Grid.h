...
private:
  ...
  //  int previousScore;                      // Điểm số của trạng thái trước đó (phục vụ Undo điểm)
    ...
    void saveState();                       // Lưu lại trạng thái bảng và điểm số hiện tại trước khi thực hiện nước đi mới
    void cleanupBoard();                    // Giải phóng bộ nhớ của các đối tượng Tile đang cấp phát động trên bảng
	// Lưu trữ nhiều trạng thái của bàn cờ để phục vụ tính năng Undo
    struct BoardState {
        std::vector<std::vector<int>> board;
        int score;
    };
    std::vector<BoardState> history;   // lưu nhiều trạng thái

    static const int MAX_POWER = 5;    // giới hạn tối đa mọi power
public:
    ...
    // Lấy số lượng powers còn lại
    int getUndoCount() const;
    int getSwapCount() const;
    int getDeleteCount() const;

    // --- Các hàm Setter và Quản lý Power-ups ---
    void addUndo(int amount = 1);
    void addSwap(int amount = 1);
    void addDelete(int amount = 1);
