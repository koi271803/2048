...
void Challenge::initChapterData() {
    // Chapter 1: 4x4 - Reach 256 - 200 moves - Không có Power
    chapters.push_back({ 256,  200, 4,  0, 0, 0 });

    // Chapter 2: 4x4 - Reach 1024 - 600 moves - Chỉ Undo
    chapters.push_back({ 1024, 600, 4,  2, 0, 0 });

    // Chapter 3: 5x5 - Reach 2048 - 700 moves - Undo + Swap
    chapters.push_back({ 2048, 700, 5,  1, 1, 0 });

    // Chapter 4: 5x5 - Reach 4096 - 900 moves - Undo + Swap
    chapters.push_back({ 4096, 900, 5,  1, 1, 0 });

    // Chapter 5: 6x6 - Reach 8192 - 1000 moves - Full Power
    chapters.push_back({ 8192, 1000, 6,  1, 1, 1 });
}
...
void Challenge::setHighestUnlocked(int value)
{
    if (value < 0) value = 0;
    if (value > 4) value = 4;
    highestUnlockedChapter = value;
    setTheme(currentTheme);   // cap nhat lai icon chapter
}
// XỬ LÝ SỰ KIỆN
