    ...
    movesText(fontRef, "0", 72)
{
    ...
    // Challenge: keo Moves + Score xuong cho can (vi da bo Best Score)
    if (isChallengeMode)
    {
        // Moves
        movesBoxSprite.setPosition(sf::Vector2f(90.f, 320.f));   // xuong ~100px
        movesText.setPosition(sf::Vector2f(240.f, 415.f));

        // Score
        scoreBoxSprite.setPosition(sf::Vector2f(40.f, 550.f));   // xuong ~100px
        scoreText.setPosition(sf::Vector2f(240.f, 650.f));
    }

    // Power panel + Menu
    powerupsPanelSprite.setPosition(sf::Vector2f(1500.f, 450.f));
        ...
        if (btnYes->isClicked(event, mousePos))
        {
            if (activePowerType == PowerType::UNDO)
            {
                if (grid.useUndo())
                {
                    // === THÊM ĐOẠN NÀY ===
                    if (isChallengeMode) {
                        currentMoves++;          // Trả lại 1 moves
                    }
                    else {
                        if (currentMoves > 0)
                            currentMoves--;      // Free mode: trừ lại
                    }
        ...
        if (!isChallengeMode || currentMoves > 0) {
            ...
            if (moved) {
                ...
                for (int val : merged)
                {
                    if (val == 128)
                    {
                        if ((!isChallengeMode || initialUndoCount > 0) && grid.getUndoCount() < 5)
                        {
                            grid.addUndo(1);
                    ...
                    {
                        if ((!isChallengeMode || initialSwapCount > 0) && grid.getSwapCount() < 5)
                        {
                            grid.addSwap(1);
                    ...
                    {
                        if ((!isChallengeMode || initialDeleteCount > 0) && grid.getDeleteCount() < 5)
                        {
                            grid.addDelete(1);
    ...
    for (size_t i = 0; i < powerSprites.size(); ++i) {
        ...
        if (shouldLock) {
            ...
            if (i < countTexts.size()) {
                ...
                countTexts[i].setPosition(sf::Vector2f(origX, 519.f)); 
            }
        }
        else {
            ...
            if (powerSprites[i].getGlobalBounds().contains(mousePos)) {
                ...
                if (i < countTexts.size()) {
                    ...
                    float origX = (i == 0) ? 1560.f : (i == 1) ? 1672.f : 1788.f;
                    countTexts[i].setPosition(sf::Vector2f(origX - 4.f, 519.f - 4.f));
                }
            }
            else {
                powerSprites[i].setScale(sf::Vector2f(1.0f, 1.0f));
...
void Gameplay::updatePowerCountsTexts() {
    ...
    for (int i = 0; i < 3; ++i) {
        countTexts[i].setPosition(sf::Vector2f(positionsX[i], 59.f));
